#Embedded file name: /Applications/Ableton Live 9.05 Suite.app/Contents/App-Resources/MIDI Remote Scripts/Livid_OhmModes/OhmModes.py
...
class ShiftModeComponent(ModeSelectorComponent):
    ...
    def set_mode_toggle(self, button1, button2, button3):
        if not (button1 == None or isinstance(button1, ButtonElement or MonoButtonElement)):
            raise AssertionError
            ...
            if not (button2 == None or isinstance(button2, ButtonElement or MonoButtonElement)):
                raise AssertionError
                ...
                raise button3 == None or isinstance(button3, ButtonElement or MonoButtonElement) or AssertionError
                self._mode_toggle3 != None and self._mode_toggle3.remove_value_listener(self._toggle_value_mod)
            self._mode_toggle3 = button3
            self._mode_toggle3 != None and self._mode_toggle3.add_value_listener(self._toggle_value_mod)
    ...
    def _toggle_value(self, value):
        if not self._mode_toggle1 != None:
            raise AssertionError
            raise self._mode_toggle2 != None or AssertionError
            raise self._mode_toggle3 != None or AssertionError
            if not isinstance(value, int):
                raise AssertionError
                value is self._mode_index and value is 3 and self.set_mode(self._last_mode)
            else:
                self.set_mode(0)
        else:
            self.set_mode(value)
    ...
    def set_mode(self, mode):
        if not isinstance(mode, int):
            ...
            if not mode in range(self.number_of_modes()):
                raise AssertionError
                self._last_mode = self._mode_index != mode and mode < 3 and mode
...
class OhmModesMonomodComponent(MonomodComponent):
    ...
    def _matrix_value(self, value, x, y, is_momentary):
        if not self._grid != None:
            ...
            self.is_enabled() and self._active_client._send_grid(x + self._x, y + self._y, value)
...
class OhmModes(ControlSurface):
    ...
    def _modNum_set_mode(self, modNum):
        ...
        def set_mode(mode):
            raise modNum._is_enabled == True and (isinstance(mode, int) or AssertionError)
            if not mode in range(modNum.number_of_modes()):
                raise AssertionError
