#Embedded file name: /Applications/Ableton Live 9.05 Suite.app/Contents/App-Resources/MIDI Remote Scripts/Livid_CodeRemoteScriptLinked/ShiftableTransportComponent.py
import Live
...
class ShiftableTransportComponent(TransportComponent):
        ...
        if not button == None:
            if isinstance(button, ButtonElement):
                ...
                if self._quant_toggle_button != button:
                    self._quant_toggle_button != None and self._quant_toggle_button.remove_value_listener(self._quant_toggle_value)
    ...
    def _undo_value(self, value):
        raise self._shift_pressed and (self._undo_button != None or AssertionError)
    ...
    def _redo_value(self, value):
        raise self._shift_pressed and (self._redo_button != None or AssertionError)
        if not value in range(128):
            raise AssertionError
            if self.is_enabled():
