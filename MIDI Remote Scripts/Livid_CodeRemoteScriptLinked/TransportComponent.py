#Embedded file name: /Applications/Ableton Live 9.05 Suite.app/Contents/App-Resources/MIDI Remote Scripts/Livid_CodeRemoteScriptLinked/TransportComponent.py
import Live
...
class TransportComponent(ControlSurfaceComponent):
    ...
    def set_stop_button(self, button):
        if not (button == None or isinstance(button, ButtonElement)):
            ...
            if self._stop_button != button:
                ...
                self._stop_button != None and self._stop_button.add_value_listener(self._stop_value)
            self.update()
    ...
    def set_play_button(self, button):
        if not (button == None or isinstance(button, ButtonElement)):
            ...
            if self._play_button != button:
                ...
                self._play_button != None and self._play_button.add_value_listener(self._play_value)
            self.update()

    def set_seek_buttons(self, ffwd_button, rwd_button):
        if not (ffwd_button == None or isinstance(ffwd_button, ButtonElement)):
            raise AssertionError
            if not (rwd_button == None or isinstance(rwd_button, ButtonElement)):
                raise AssertionError
                if not self._ffwd_button != ffwd_button:
                    do_update = self._rwd_button != rwd_button
                    if not self._ffwd_button_pressed:
                        remove_timer = self._rwd_button_pressed
                        ...
                        if self._rwd_button != rwd_button:
                            self._rwd_button != None and self._rwd_button.remove_value_listener(self._rwd_value)
                        self._rwd_button_pressed = False
                        self._rwd_button = rwd_button
                        self._rwd_button != None and self._rwd_button.add_value_listener(self._rwd_value)
                do_update and remove_timer and self._unregister_timer_callback(self._on_timer)
            self.update()

    def set_nudge_buttons(self, up_button, down_button):
        if not (up_button == None or isinstance(up_button, ButtonElement) and up_button.is_momentary()):
            raise AssertionError
            if not (down_button == None or isinstance(down_button, ButtonElement) and down_button.is_momentary()):
                raise AssertionError
                if self._nudge_up_button != None:
                    self._nudge_up_button.remove_value_listener(self._nudge_up_value)
                ...
                self._nudge_down_button != None and self._nudge_down_button.remove_value_listener(self._nudge_down_value)
            self._nudge_down_button = down_button
            self._nudge_down_button != None and self._nudge_down_button.add_value_listener(self._nudge_down_value)
        self.update()
    ...
    def set_record_button(self, button):
        if not (button == None or isinstance(button, ButtonElement)):
            ...
            if self._record_button != button:
                ...
                self._record_button != None and self._record_button.add_value_listener(self._record_value)
            self.update()
    ...
    def set_tap_tempo_button(self, button):
        if not (button == None or isinstance(button, ButtonElement)):
            ...
            if self._tap_tempo_button != button:
                ...
                self._tap_tempo_button != None and self._tap_tempo_button.add_value_listener(self._tap_tempo_value)
            self.update()
    ...
    def set_loop_button(self, button):
        if not (button == None or isinstance(button, ButtonElement)):
            ...
            if self._loop_button != button:
                ...
                self._loop_button != None and self._loop_button.add_value_listener(self._loop_value)
            self.update()

    def set_punch_buttons(self, in_button, out_button):
        if not (in_button == None or isinstance(in_button, ButtonElement)):
            raise AssertionError
            if not (out_button == None or isinstance(out_button, ButtonElement)):
                raise AssertionError
                if self._punch_in_button != None:
                    self._punch_in_button.remove_value_listener(self._punch_in_value)
                ...
                self._punch_out_button != None and self._punch_out_button.remove_value_listener(self._punch_out_value)
            self._punch_out_button = out_button
            self._punch_out_button != None and self._punch_out_button.add_value_listener(self._punch_out_value)
        self.update()
    ...
    def set_metronome_button(self, button):
        if not (button == None or isinstance(button, ButtonElement)):
            ...
            if self._metronome_button != button:
                ...
                self._metronome_button != None and self._metronome_button.add_value_listener(self._metronome_value)
    ...
    def set_overdub_button(self, button):
        if not (button == None or isinstance(button, ButtonElement)):
            ...
            if self._overdub_button != button:
                ...
                self._overdub_button != None and self._overdub_button.add_value_listener(self._overdub_value)
            self._on_overdub_changed()
            self.update()

    def set_tempo_control(self, control, fine_control = None):
        if not (control == None or isinstance(control, EncoderElement) and control.message_map_mode() is Live.MidiMap.MapMode.absolute):
            raise AssertionError
            if not (fine_control == None or isinstance(fine_control, EncoderElement) and fine_control.message_map_mode() is Live.MidiMap.MapMode.absolute):
                raise AssertionError
                if self._tempo_control != None:
                    self._tempo_control.remove_value_listener(self._tempo_value)
                ...
                self._tempo_fine_control != None and self._tempo_fine_control.remove_value_listener(self._tempo_fine_value)
            self._tempo_fine_control = fine_control
            self._fine_tempo_needs_pickup = True
            self._prior_fine_tempo_value = -1
            self._tempo_fine_control != None and self._tempo_fine_control.add_value_listener(self._tempo_fine_value)
        self.update()
    ...
    def set_song_position_control(self, control):
        if not (control == None or isinstance(control, EncoderElement)):
            ...
            if self._song_position_control != control:
                ...
                self._song_position_control != None and self._song_position_control.add_value_listener(self._song_position_value)
            self.update()
    ...
    def _stop_value(self, value):
        if not self._stop_button != None:
            raise AssertionError
            if not isinstance(value, int):
                raise AssertionError
                self.song().is_playing = self.is_enabled() and (value != 0 or not self._stop_button.is_momentary()) and False

    def _play_value(self, value):
        if not self._play_button != None:
            raise AssertionError
            if not isinstance(value, int):
                raise AssertionError
                self.song().is_playing = self.is_enabled() and (value != 0 or not self._play_button.is_momentary()) and True
                ...
                if self.is_enabled():
                    ...
                    if self._ffwd_button.is_momentary():
                        if self._ffwd_button_pressed is not (value != 0):
                            ...
                            self._rwd_button_pressed or self._ffwd_button_pressed and self._register_timer_callback(self._on_timer)
                        else:
                            self._unregister_timer_callback(self._on_timer)
                ...
                if self.is_enabled():
                    ...
                    if self._rwd_button.is_momentary():
                        if self._rwd_button_pressed is not (value != 0):
                            ...
                            self._ffwd_button_pressed or self._rwd_button_pressed and self._register_timer_callback(self._on_timer)
                        else:
                            self._unregister_timer_callback(self._on_timer)

    def _nudge_down_value(self, value):
        if not self._nudge_down_button != None:
            raise AssertionError
            raise value in range(128) or AssertionError
            self.song().nudge_down = self.is_enabled() and value != 0

    def _nudge_up_value(self, value):
        if not self._nudge_up_button != None:
            raise AssertionError
            raise value in range(128) or AssertionError
            self.song().nudge_up = self.is_enabled() and value != 0

    def _loop_value(self, value):
        if not self._loop_button != None:
            raise AssertionError
            if not isinstance(value, int):
                raise AssertionError
                self.song().loop = self.is_enabled() and (value != 0 or not self._loop_button.is_momentary()) and not self.song().loop

    def _punch_in_value(self, value):
        if not self._punch_in_button != None:
            raise AssertionError
            if not isinstance(value, int):
                raise AssertionError
                self.song().punch_in = self.is_enabled() and (value != 0 or not self._punch_in_button.is_momentary()) and not self.song().punch_in

    def _punch_out_value(self, value):
        if not self._punch_out_button != None:
            raise AssertionError
            if not isinstance(value, int):
                raise AssertionError
                self.song().punch_out = self.is_enabled() and (value != 0 or not self._punch_out_button.is_momentary()) and not self.song().punch_out

    def _record_value(self, value):
        if not self._record_button != None:
            raise AssertionError
            if not isinstance(value, int):
                raise AssertionError
                self.song().record_mode = self.is_enabled() and (value != 0 or not self._record_button.is_momentary()) and not self.song().record_mode

    def _tap_tempo_value(self, value):
        if not self._tap_tempo_button != None:
            raise AssertionError
            if not isinstance(value, int):
                raise AssertionError
                self.is_enabled() and (value != 0 or not self._tap_tempo_button.is_momentary()) and self.song().tap_tempo()
    ...
    def _tempo_fine_value(self, value):
        if not self._tempo_fine_control != None:
            raise AssertionError
            if not value in range(128):
                raise AssertionError
                if self.is_enabled():
                    if self._fine_tempo_needs_pickup:
                        range_max = self._prior_fine_tempo_value in range(128) and max(value, self._prior_fine_tempo_value)
                        range_min = min(value, self._prior_fine_tempo_value)
                        self._fine_tempo_needs_pickup = 64 in range(range_min, range_max + 1) and False
            else:
                raise self._prior_fine_tempo_value in range(128) or AssertionError
    ...
    def _song_position_value(self, value):
        raise self._song_position_control != None or AssertionError
        raise isinstance(value, int) or AssertionError
        debug_print('Code Missing')
        raise False or AssertionError

    def _metronome_value(self, value):
        if not self._metronome_button != None:
            raise AssertionError
            if not value in range(128):
                raise AssertionError
                self.song().metronome = self.is_enabled() and (value != 0 or not self._metronome_button.is_momentary()) and not self.song().metronome

    def _overdub_value(self, value):
        if not self._overdub_button != None:
            raise AssertionError
            if not value in range(128):
                raise AssertionError
                self.song().overdub = self.is_enabled() and (value != 0 or not self._overdub_button.is_momentary()) and not self.song().overdub
