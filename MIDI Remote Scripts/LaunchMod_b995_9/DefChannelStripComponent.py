#Embedded file name: /Applications/Ableton Live 9 Standard.app/Contents/App-Resources/MIDI Remote Scripts/LaunchMod_b995_9/DefChannelStripComponent.py
import Live
...
class DefChannelStripComponent(ChannelStripComponent):
    ...
    def _default_volume_value(self, value):
        if not self._default_volume_button != None:
            ...
            if not value in range(128):
                raise AssertionError
                volume = self.is_enabled() and self._track != None and (value != 0 or not self._default_volume_button.is_momentary()) and self._track.mixer_device.volume
    ...
    def _default_panning_value(self, value):
        if not self._default_panning_button != None:
            ...
            if not value in range(128):
                raise AssertionError
                panning = self.is_enabled() and self._track != None and (value != 0 or not self._default_panning_button.is_momentary()) and self._track.mixer_device.panning
    ...
    def _default_send1_value(self, value):
        if not self._default_send1_button != None:
            ...
            if not value in range(128):
                raise AssertionError
                send1 = self.is_enabled() and self._track != None and len(self._track.mixer_device.sends) > 0 and (value != 0 or not self._default_send1_button.is_momentary()) and self._track.mixer_device.sends[0]
    ...
    def _default_send2_value(self, value):
        if not self._default_send2_button != None:
            ...
            if not value in range(128):
                raise AssertionError
                send2 = self.is_enabled() and self._track != None and len(self._track.mixer_device.sends) > 1 and (value != 0 or not self._default_send2_button.is_momentary()) and self._track.mixer_device.sends[1]
