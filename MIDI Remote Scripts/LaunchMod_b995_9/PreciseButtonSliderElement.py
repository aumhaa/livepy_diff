#Embedded file name: /Applications/Ableton Live 9 Standard.app/Contents/App-Resources/MIDI Remote Scripts/LaunchMod_b995_9/PreciseButtonSliderElement.py
...
class PreciseButtonSliderElement(ButtonSliderElement):
    ...
    def send_value(self, value):
        if not (self._disabled or value != None):
            raise AssertionError
            raise isinstance(value, int) or AssertionError
            if not value in range(128):
                raise AssertionError
                value != self._last_sent_value and self._mode == SLIDER_MODE_SINGLE and ButtonSliderElement.send_value(self, value)
    ...
    def _button_value(self, value, sender):
        if not isinstance(value, int):
            ...
            if not sender in self._buttons:
                ...
                self._parameter_to_map_to.value = self._parameter_to_map_to != None and self._value_map[index_of_sender]
