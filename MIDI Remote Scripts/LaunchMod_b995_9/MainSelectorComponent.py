#Embedded file name: /Applications/Ableton Live 9 Beta.app/Contents/App-Resources/MIDI Remote Scripts/LaunchMod_b995_9/MainSelectorComponent.py
...
class MainSelectorComponent(ModeSelectorComponent):
    ...
    def __init__(self, matrix, top_buttons, side_buttons, config_button, script):
        if not isinstance(matrix, ButtonMatrixElement):
            ...
            START_IN_MOD is True and self._set_protected_mode_index(4)
    ...
    def _setup_mixer(self, as_active):
        raise isinstance(as_active, type(False)) or AssertionError
            ...
            if not self._modes_buttons.count(sender) == 1:
                ...
                if self._script._host.is_enabled() != True:
                    if value is not 0 or not sender.is_momentary():
                        (self._modes_buttons[1]._last_received_value > 0 and self._modes_buttons.index(sender) == 2 or self._modes_buttons[2]._last_received_value > 0 and self._modes_buttons.index(sender) == 1) and self._script._host._active_client != None and self.set_mode(4)
                    else:
