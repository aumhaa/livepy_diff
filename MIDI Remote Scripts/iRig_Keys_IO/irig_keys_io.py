...
class IRigKeysIO(ControlSurface):
    ...
    def _create_controls(self):
        self._encoders = ButtonMatrixElement(rows=[[ EncoderElement(MIDI_CC_TYPE, 0, identifier, map_mode=Live.MidiMap.MapMode.absolute, name='Volume_Encoder_{}'.format(index)) for index, identifier in enumerate(xrange(12, 20)) ]], name='Volume_Encoders')
        ...
        self._pads = ButtonMatrixElement(rows=[[ ButtonElement(True, MIDI_NOTE_TYPE, PAD_CHANNEL, identifier, name='Pad_{}'.format(index)) for index, identifier in enumerate(PAD_IDS) ]])
