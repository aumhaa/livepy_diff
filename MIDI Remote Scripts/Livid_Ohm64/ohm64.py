#Embedded file name: /Applications/Ableton Live 9.05 Suite.app/Contents/App-Resources/MIDI Remote Scripts/Livid_Ohm64/ohm64.py
...
class ohm64(ControlSurface):
    ...
    def __init__(self, c_instance):
        ...
        with self.component_guard():
            self.log_message(time.strftime('%d.%m.%Y %H:%M:%S', time.localtime()) + '--------------= Ohm64 opened =--------------')
    ...
    def disconnect(self):
        ...
        self.log_message(time.strftime('%d.%m.%Y %H:%M:%S', time.localtime()) + '--------------= Ohm64 log closed =--------------')
