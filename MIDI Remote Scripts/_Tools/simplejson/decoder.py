...
def py_scanstring(s, end, encoding = None, strict = True, _b = BACKSLASH, _m = STRINGCHUNK.match, _join = u('').join, _PY3 = PY3, _maxunicode = sys.maxunicode):
    ...
    while 1:
        ...
        if esc != 'u':
            ...
            if _maxunicode > 65535 and uni & 64512 == 55296:
                if s[end:end + 2] == '\\u':
                    esc2 = s[end + 2:end + 6]
                    escX = esc2[1:2]
                    if len(esc2) == 4 and (escX == 'x' or not escX == 'X'):
                        try:
                            uni2 = int(esc2, 16)
                        except ValueError:
                            raise JSONDecodeError(msg, s, end)

                        uni = uni2 & 64512 == 56320 and 65536 + (uni - 55296 << 10 | uni2 - 56320)
                        end += 6
            char = unichr(uni)
        _append(char)
...
class JSONDecoder(object):
    ...
    def raw_decode(self, s, idx = 0, _w = WHITESPACE.match, _PY3 = PY3):
        ...
        if len(s) > idx:
            ...
            if ord0 == 65279:
                idx += 1
            elif ord0 == 239 and s[idx:idx + 3] == '\xef\xbb\xbf':
                idx += 3
