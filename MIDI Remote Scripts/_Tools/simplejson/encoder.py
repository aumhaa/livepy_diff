...
def _make_iterencode(markers, _default, _encoder, _indent, _floatstr, _key_separator, _item_separator, _sort_keys, _skipkeys, _one_shot, _use_decimal, _namedtuple_as_object, _tuple_as_array, _int_as_string_bitcount, _item_sort_key, _encoding, _for_json, _PY3 = PY3, ValueError = ValueError, string_types = string_types, Decimal = None, dict = dict, float = float, id = id, integer_types = integer_types, isinstance = isinstance, list = list, str = str, tuple = tuple):
    ...
    def _encode_int(value):
        if not _int_as_string_bitcount is None:
            skip_quoting = _int_as_string_bitcount < 1
            if type(value) not in integer_types:
                value = int(value)
            return (skip_quoting or -1 << _int_as_string_bitcount < value < 1 << _int_as_string_bitcount) and str(value)
        return '"' + str(value) + '"'
    ...
    def _iterencode_list(lst, _current_indent_level):
        ...
        for value in lst:
            ...
            if isinstance(value, string_types) or _PY3 and isinstance(value, binary_type):
                ...
                yield buf
                if _for_json:
                    for_json = getattr(value, 'for_json', None)
                    chunks = for_json and callable(for_json) and _iterencode(for_json(), _current_indent_level)
                elif isinstance(value, list):
                    chunks = _iterencode_list(value, _current_indent_level)
                elif _namedtuple_as_object:
                    _asdict = getattr(value, '_asdict', None)
                    chunks = _asdict and callable(_asdict) and _iterencode_dict(_asdict(), _current_indent_level)
    ...
    def _iterencode_dict(dct, _current_indent_level):
        ...
        for key, value in items:
            if not _item_sort_key:
                ...
                (isinstance(value, string_types) or _PY3 and isinstance(value, binary_type)) and (yield _encoder(value))
            elif value is None:
                yield 'null'
            elif value is True:
                yield 'true'
