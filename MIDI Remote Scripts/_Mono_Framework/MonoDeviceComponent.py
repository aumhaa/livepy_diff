...
class MonoDeviceComponent(DeviceComponent):
    ...
    def _assign_parameters(self, host, *a):
        ...
        if class_name in self._device_banks.keys():
            if not class_name in self._device_best_banks.keys():
                ...
                if host._parameter_controls is not None:
    ...
    def update(self):
        if self.is_enabled():
            if self._device != None:
                ...
                for host in self._parent._active_host:
                    if host.is_enabled() and host._parameter_controls is not None and len(host._parameter_controls) > 0:
        ...
        if self.is_enabled():
            for host in self._parent._active_host:
                if host.is_enabled():
                    if host._parameter_controls is not None and len(host._parameter_controls) > 0:
    ...
    def _update_params(self):
        ...
        for host in self._parent._host._hosts:
            if host._parameter_controls is not None and len(host._parameter_controls) > count:
