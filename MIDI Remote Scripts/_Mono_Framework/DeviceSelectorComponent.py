#Embedded file name: /Applications/Ableton Live 9 Beta.app/Contents/App-Resources/MIDI Remote Scripts/_Mono_Framework/DeviceSelectorComponent.py
import Live
...
class NewDeviceSelectorComponent(ControlSurfaceComponent):
    ...
    def select_device(self, index):
        if self.is_enabled():
            ...
            if preset is not None and isinstance(preset, Live.Device.Device):
        ...
        for track in self.song().tracks:
            for device in self.enumerate_track_device(track):
                for index, entry in enumerate(self._device_registry):
                    ...
                    self._device_registry[index] = match(key, str(device.name)) != None and device
