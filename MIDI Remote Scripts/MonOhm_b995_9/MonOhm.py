#Embedded file name: /Applications/Ableton Live 9 Beta.app/Contents/App-Resources/MIDI Remote Scripts/MonOhm_b995_9/MonOhm.py
...
class ShiftModeComponent(ModeSelectorComponent):
    ...
    def _toggle_value(self, value, side):
        raise self._mode_toggle1 != None or AssertionError
...
class MonOhm(ControlSurface):
    ...
    def monomod_mode_update(self):
        if self._monomod_mode._mode_index == 0:
            ...
            if self._host._active_client == None:
                ...
                self._host._set_nav_buttons([self._menu[0],
                 self._menu[3],
                 self._menu[4],
                 self._menu[5]])
    ...
    def disconnect(self):
        ...
        self.log_message('--------------=  ' + str(self._host_name) + ' ' + str(self._monomod_version) + ' log closed =--------------')
    ...
    def get_session_offsets(self):
        if self._is_split is True:
            return [self._session.track_offset(),
             self._session.scene_offset(),
             self._session2.track_offset(),
             self._session2.scene_offset()]
        elif self._is_split is False:
            return [self._session_main.track_offset(),
             self._session_main.scene_offset(),
             self._session_main.track_offset() + 4,
             self._session_main.scene_offset()]

    def set_split_mixer(self, is_split):
        if not isinstance(is_split, type(False)):
            raise AssertionError
            if is_split != self._is_split:
                self._mixer._track_offset = is_split is True and self._session._track_offset
            else:
